...
void handelWater() {
	if (currentMillis - previousMillis >= duration*1000 && watering_flag == 1 ) {
		previousMillis = currentMillis;
		Serial.println("if1");
		watering_flag = 0;
	}

	else if (hour()==start_h && minute()==start_min && timeStatus()==timeSet && currentMillis-previousMillis<=1000) {
		watering_flag == 1;
		Serial.println("elseif2");
...
//	Serial.printf("Got IP: %s\r\n", ipInfo.ip.toString().c_str());
	Serial.printf("test");//to do remove
  ...
  if (server.hasArg("Duration")) {
    String dtmp = server.arg("Duration");
		switch(handleDuration(dtmp)) {
			...
			errmsg += "<strong style=\"color: red;\">";
			errmsg += dtmp;
			errmsg += "Duration must be smaller than 4294967295";
			errmsg += "</strong>";
			break;
